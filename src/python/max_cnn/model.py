...
class CNNClassifier(Module):
    ...
    def __init__(
        ...
        self.conv1_W = ops.constant(
            weights['conv1_W'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        self.conv1_b = ops.constant(
            weights['conv1_b'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        ...
        self.conv2_W = ops.constant(
            weights['conv2_W'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        self.conv2_b = ops.constant(
            weights['conv2_b'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        
        # FC layer 1: flattened → 128
        self.fc1_W = ops.constant(
            weights['fc1_W'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        self.fc1_b = ops.constant(
            weights['fc1_b'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        
        # FC layer 2 (output): 128 → num_classes
        self.fc2_W = ops.constant(
            weights['fc2_W'].astype(np.float32),
            dtype=dtype,
            device=device
        )
        self.fc2_b = ops.constant(
            weights['fc2_b'].astype(np.float32),
            dtype=dtype,
            device=device
        )
