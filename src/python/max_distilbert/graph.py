...
class DistilBertClassifier(Module):
    ...
    def __init__(
        ...
        self.pre_classifier_weight = weights.pre_classifier.weight.allocate(DType.float32).cast(
            dtype
        )
    ...
    def __call__(
        ...
        pooled_output = (
            ops.matmul(cls_output, ops.transpose(self.pre_classifier_weight, 1, 0))
            + self.pre_classifier_bias
        )
        pooled_output = ops.relu(pooled_output)

        # Final classification layer
        logits = (
            ops.matmul(pooled_output, ops.transpose(self.classifier_weight, 1, 0))
