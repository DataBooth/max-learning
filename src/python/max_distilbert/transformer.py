...
class DistilBertAttention(Module):
    ...
    def __call__(self, hidden_states: TensorValue, attention_mask: TensorValue) -> TensorValue:
        ...
        query = ops.reshape(
            query, [batch_size, seq_length, self.num_attention_heads, self.attention_head_size]
        )
        query = ops.permute(query, [0, 2, 1, 3])  # [batch, num_heads, seq_len, head_size]

        key = ops.reshape(
            key, [batch_size, seq_length, self.num_attention_heads, self.attention_head_size]
        )
        key = ops.permute(key, [0, 2, 1, 3])

        value = ops.reshape(
            value, [batch_size, seq_length, self.num_attention_heads, self.attention_head_size]
        )
        ...
        if attention_mask is not None:
            ...
            additive_mask = inverted_mask * ops.constant(
                -10000.0, dtype=inverted_mask.dtype, device=inverted_mask.device
            )
...
class DistilBertFeedForward(Module):
    ...
    def __call__(self, hidden_states: TensorValue) -> TensorValue:
        ...
        hidden_states = (
            ops.matmul(hidden_states, ops.transpose(self.lin1_weight, 1, 0)) + self.lin1_bias
        )
        hidden_states = ops.gelu(hidden_states)

        # Second projection
        hidden_states = (
            ops.matmul(hidden_states, ops.transpose(self.lin2_weight, 1, 0)) + self.lin2_bias
        )
...
class DistilBertTransformerBlock(Module):
    ...
    def __init__(self, weights, layer_idx, config, dtype, device):
        ...
        # Output layer norm
        self.output_layer_norm_weight = weights.output_layer_norm.weight.allocate(
            DType.float32
        ).cast(dtype)
        self.output_layer_norm_bias = weights.output_layer_norm.bias.allocate(DType.float32).cast(
            dtype
        )
    ...
    def __init__(self, weights, config, dtype, device):
        ...
        for i in range(self.num_layers):
            layer = DistilBertTransformerBlock(
                weights.layer[i], layer_idx=i, config=config, dtype=dtype, device=device
            )
