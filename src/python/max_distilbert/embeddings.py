...
class DistilBertEmbeddings(Module):
    ...
    def __init__(
        ...
        # Word embeddings
        self.word_embeddings = weights.word_embeddings.weight.allocate(DType.float32).cast(dtype)

        # Position embeddings
        self.position_embeddings = weights.position_embeddings.weight.allocate(DType.float32).cast(
            dtype
        )

        # LayerNorm
        self.LayerNorm_weight = weights.LayerNorm.weight.allocate(DType.float32).cast(dtype)
        self.LayerNorm_bias = weights.LayerNorm.bias.allocate(DType.float32).cast(dtype)
