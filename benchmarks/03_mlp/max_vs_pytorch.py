...
def main():
    ...
    print(
        f"Architecture: {config['model']['input_size']} → {config['model']['hidden_size1']} → {config['model']['hidden_size2']} → {config['model']['output_size']}"
    )
    ...
    test_data = np.random.randn(
        config["model"]["batch_size"], config["model"]["input_size"]
    ).astype(np.float32)
    ...
    if max_results and pytorch_results:
        ...
        if np.allclose(max_pred, pytorch_pred, rtol=1e-4, atol=1e-5):
            ...
            for i in range(min(3, len(test_input))):
                print(
                    f"  Sample {i + 1}: PyTorch={pytorch_pred[i, 0]:.6f}, MAX={max_pred[i, 0]:.6f}"
                )
