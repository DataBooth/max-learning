...
def build_elementwise_graph(device_type: str, size: int) -> Graph:
    ...
    with Graph(f"elementwise_{device_type}_{size}", input_types=[input_spec]) as graph:
        ...
        multiplier = ops.constant(
            np.full(size, 2.0, dtype=np.float32), dtype=DType.float32, device=x.device
        )

        offset = ops.constant(
            np.full(size, 1.0, dtype=np.float32), dtype=DType.float32, device=x.device
        )
...
def main():
    ...
    print("Testing sizes from 4 to 8M elements...\n")
    ...
    for i, size in enumerate(sizes, 1):
        ...
        if cpu_time and gpu_time:
            ...
            results.append(
                {"size": size, "cpu_ms": cpu_time, "gpu_ms": gpu_time, "speedup": speedup}
            )
