...
def build_elementwise_graph(
    device_type: str, multiplier_val: float, offset_val: float, input_size: int
) -> Graph:
    ...
    with Graph(f"elementwise_{device_type}", input_types=[input_spec]) as graph:
        ...
        offset = ops.constant(
            np.full(input_size, offset_val, dtype=np.float32), dtype=DType.float32, device=x.device
        )
...
def main():
    ...
    if match:
        ...
        print("\nOperations tested:")
        print("  - ops.mul (element-wise multiplication)")
        print("  - ops.add (element-wise addition)")
        print("  - ops.relu (element-wise activation)")
    else:
