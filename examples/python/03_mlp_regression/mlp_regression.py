...
def main():
    ...
    print(
        f"Architecture: Input({input_size}) → FC({hidden_size1})+ReLU → FC({hidden_size2})+ReLU → FC({output_size})\n"
    )
    ...
    print("   ✓ Weights loaded (trained with R²=0.79, RMSE=$52.9k)\n")
    ...
    print(
        f"   ✓ Features standardised (mean={scaler_params['mean'][0]:.2f}, scale={scaler_params['scale'][0]:.2f})\n"
    )
    ...
    for i in range(len(X_sample)):
        ...
        print(
            f"#{i + 1:<7} ${true_val * 100:.1f}k{'':<8} ${pred_val * 100:.1f}k{'':<8} ${error * 100:.1f}k"
        )
